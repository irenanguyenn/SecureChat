...
function isWhitelistedDomain(url) {
    try {
        ...
        // Check exact match or subdomain (like 'sub.google.com')
...
function linkify(text) {
    ...
    return text.replace(urlRegex, (url) => {
        try {
            if (isBlacklistedDomain(url)) return `[Blocked]`;
            if (!isWhitelistedDomain(url)) return `[Blocked]`;
...
async function loadFriends() {
    ...
    friendsRef.once("value", (snapshot) => {
        snapshot.forEach((childSnapshot) => {
            ...
            horizontalRow.style.alignItems = "center";
            horizontalRow.style.justifyContent = "space-between";

            horizontalRow.appendChild(wrapper);
            horizontalRow.appendChild(removeBtn);

            listItem.appendChild(horizontalRow);
...
firebase.auth().onAuthStateChanged(user => {
    if (user) {
        ...
        userRef.once("value").then(snapshot => {
            ...
            if (username) {
                sessionStorage.setItem("loggedInUsername", username);
                document.getElementById("signed-in-user").textContent = `Signed in as ${username}`;
                loadFriendRequests();
                loadFriends();
                setupHeartbeat();
            } else {
...
async function sendMessage() {
    ...
    switch (currentFormat) {
        ...
        case "normal":
        default:
            // Preserve valid links inserted via the button
            if (message.includes("<a") && message.includes("</a>")) {
                // Don't escape if it's a full <a> tag
                break;
            }
    ...
    if (/^https?:\/\/[^\s]+$/.test(message.trim())) {
        ...
        refToUse.push({
          username: username,
          message: message, // escape plain text to prevent XSS
          timestamp: firebase.database.ServerValue.TIMESTAMP
        });
      }
...
function displayMessage(username, message, timestamp) {
    ...
    const isPDF = message.includes(".pdf") && message.includes("href=");
    const isLink = message.includes("<a") && !isPDF;
    const prefix = escapeHTML(username) + ": ";

    if (isImage) {
        // Render the img HTML as-is
        messageElement.innerHTML = prefix + message;
    } else if (isPDF) {
        // Extract URL from <a href="">
        const match = message.match(/href="([^"]+\.pdf)"/);
        if (match) {
            const pdfURL = match[1];
            messageElement.innerHTML = `${prefix}<iframe src="${pdfURL}" width="250" height="300" style="border-radius: 10px; border: none;"></iframe>`;
        } else {
            messageElement.innerHTML = prefix + message;
        }
    } else if (isLink) {
        ...
        const safeLinked = linkify(decoded);
        messageElement.innerHTML = safeLinked;
    }    
...
if (linkBtn && messageInputBox) {
    linkBtn.addEventListener("click", () => {
        ...
        if (url && isValidURL(url.trim()) && !isBlacklistedDomain(url) && isWhitelistedDomain(url)) {
            const safeURL = url.trim();
            messageInputBox.value += safeURL; // insert raw URL only
        } else {
            alert("Invalid URL");
        }        
    });
